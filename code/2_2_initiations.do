...
count if drug_init_date>last_visit & drug_init_date<. // 186
count if generic_init_date>last_visit & generic_init_date<. // 168

count if drug_start==1 & inlist(drug_category,250,390) //& drug_start_date<=last_visit 
...
keep if drug_start==1 & inlist(drug_category,250,390) 
// 2025-01-15 added
drop if drug_start_date>last_visit & drug_start_date<.
...
drop nmiss
save temp\start_btdmards_1st_per_visit_drug, replace 
restore 

// 2025-01-15 added & generic_start_date<=last_visit
...
drop if generic_start_date>last_visit & generic_start_date<.
...
save temp\start_btdmards_1st_per_visit_generic, replace
restore 
...
drop _m 
save temp\start_btdmards_1st_per_visit, replace 
...
use 2_3_keyvisitvars_$datacut.dta, clear
...
save temp\keyvisits_simple, replace 


/////////////////////////////////////////////////////////////////////
// STEP A. use simplified visitdate data to add cdmard use 
use temp\keyvisits_simple, clear 

merge 1:1 subject_number visitdate using temp\start_btdmards_1st_per_visit
...
save temp\keyvisits_starts, replace 
...
// 2025-01-15 also added limitation here 
drop if drug_start_date>last_visit & drug_start_date<.
drop if generic_start_date>last_visit & generic_start_date<.
...
by subject_number : replace prev_reason_`i'_category=reason_`i'_category[_n-1]  if init_`x'==1 & nhx_b_ts_generic>0 // 2024-11-04 fixed from 1 to `i'
...
// 2024-10-30 adding more biosimilars 
foreach y in $btsdmards_drug_list amjevita cyltezo hadlima hulio hyrimoz yusimry ixifi riabni erelzi rituxan_bs{
...
save temp\\`y'_init_date, replace 
restore 
...
save temp\\`y'_stop_date, replace 
restore 
}
...
// test start data 
use temp\enbrel_init_date, clear
...
foreach y in $btsdmards_drug_list adalimumab etanercept golimumab infliximab rituximab tofacitinib{
    display "`y'"
// Add C to data B 
use temp\keyvisits_starts, clear 
merge m:1 subject_number using temp\\`y'_init_date
drop _m 
...
// Add D to B & C 
merge 1:1 subject_number visitdate using temp\\`y'_stop_date
drop if _m==2
drop _m 
...
replace switch=. if "`y'"=="adalimumab" & switch==1  & numstart==1 & (strpos(namestart,"amjevita")|strpos(namestart,"humira")|strpos(namestart,"cyltezo")|strpos(namestart,"hadlima")|strpos(namestart,"idacio")|strpos(namestart,"hyrimoz")|strpos(namestart,"yusimry")|strpos(namestart,"hulio"))
...
replace switch=. if "`y'"=="infliximab" & switch==1  & numstart==1 & (strpos(namestart,"avsola")|strpos(namestart,"inflectra")|strpos(namestart, "remicade")|strpos(namestart, "renflexis")|strpos(namestart, "ixifi"))
replace switch=. if "`y'"=="rituximab" & switch==1  & numstart==1 & (strpos(namestart,"rituxan")|strpos(namestart, "ruxience")|strpos(namestart,"truxima")|strpos(namestart, "riabni"))
...
save temp\init_`y', replace 
}

use temp\init_infliximab, clear 
tab switchname 
...
//LG 2024-10-31: riabni stop date has no observations for 2024-10-01 datacut, check for 2024-11-01 datacut for updated numbers 

foreach y in amjevita cyltezo hadlima hulio hyrimoz yusimry ixifi  erelzi rituxan_bs{
    display "`y'"
// Add C to data B 
use temp\keyvisits_starts, clear 
merge m:1 subject_number using temp\\`y'_init_date
...
// 2024-10-31 exclude riabni, too few cases, no obs in stop date data yet.
use temp\init_adalimumab, clear 
foreach y in etanercept golimumab infliximab rituximab tofacitinib orencia humira amjevita cyltezo hadlima hulio hyrimoz yusimry kineret cimzia enbrel erelzi simponi simponi_aria remicade avsola inflectra renflexis ixifi remicade_bs rituxan ruxience truxima  rituxan_bs kevzara actemra olumiant xeljanz xeljanz_xr rinvoq {
    append using "temp\init_`y'.dta"
}

groups druggrp if grpindexn==1
drop start_details

*save temp\2_2_allinits_2024-10-31, replace 

save 2_2_allinits_$datacut, replace 


// eliminate temporary files
foreach y in adalimumab etanercept golimumab infliximab rituximab tofacitinib amjevita  cyltezo hadlima hulio hyrimoz yusimry ixifi riabni erelzi  $btsdmards_drug_list {
cap erase temp\\`y'_init_date.dta 
cap erase temp\\`y'_stop_date.dta
cap erase temp\init_`y'.dta
}
